...
function bubbleChart() {
  ...
  var yearCenters = {
    2008: { x:  (width / 5) * 1, y: height / 2 },
    2009: { x: (width / 5) * 2 , y: height / 2 },
    2010: { x: (width / 5) * 3, y: height / 2 },
 2011: { x: (width / 5) * 4,  y: height / 2 }
  };
  ...
  var fillColor = d3.scale.ordinal()
    ...
    .range(['#4FC1FE', '#5F6EFE', '#FFCD46', '#FFAB46', '#02476D','#0016E2','#FFBA00','#FF8B00','#945000','#060F66']);
 ...
 force.on('tick', function (e) {
  ...
  function moveToYears(alpha) {
    return function (d) {     
      ...
      else if(d.year=='ripio') {
        tipo = 2009
      }     
      else if(d.year=='tierra') {
        tipo = 2010
      } else  {
        tipo = 2011
      }    
  ...
  function showYears() {
    ...
    years.enter().append('text')
      ...
      for (var i = myNodes.length - 1; i >= 0; i--) {
        ...
        if (myNodes[i].year == tipo) {
          Cantidad = Cantidad + 1
        };
        /* EN EL CASO DE LOS DEMAS*/
        if (tipo == 'N/S') {

          if (myNodes[i].year != 'asfalto' && myNodes[i].year != 'ripio'&& myNodes[i].year != 'tierra') {
        ...
        return  tipo + ' (' + Cantidad  + '/'+  myNodes.length + ')'; });
  }
...
function splitBubbles2() {
 ...
 force.on('tick', function (e) {
      ...
      bubbles.each(moveToLoteamiento(e.alpha))
        .attr('cx', function (d) { return d.x; })
        .attr('cy', function (d) { return d.y; });
    });
   ...
   * node's charge force to also impact final location.
   */
  function moveToLoteamiento(alpha) {
    return function (d) {     
   ...
   * Hides Year title displays.
   */
  function hideLoteamiento() {
    svg.selectAll('.loteamiento').remove();
  }

  /*
   * Shows Year title displays.
   */
  function showLoteamiento() {
    // Another way to do this would be to create
    // the year texts once and then just hide them.
    var yearsData = d3.keys(LoteamientoTitleX);
    var years = svg.selectAll('.loteamiento')
      .data(yearsData);
    ...
    years.enter().append('text')
      ...
      for (var i = myNodes.length - 1; i >= 0; i--) {
        ...
        if (myNodes[i].loteamiento == tipo) {
          Cantidad = Cantidad + 1
        };
        /* EN EL CASO DE LOS DEMAS*/
        if (tipo == 'N/S') {

          if (myNodes[i].loteamiento != 'no' && myNodes[i].loteamiento != 'si') {
  ...
  function showDetail(d) {
    ...
    var content = '<span class="name">Nombre del Asentamiento: </span><span class="value">' +
                  ...
                  d.year +
                  '</span><br/> '+
                  '<span class="name">Loteamiento: </span><span class="value">' +
                  d.loteamiento +
                  '</span><br/> '+
                  '<span class="name">Ciudad: </span><span class="value">' +
                  d.org +
                  '</span><br/>' ;
  ...
  chart.toggleDisplay = function (displayName) {
   ...
   else  if (displayName === 'loteamiento') {
      splitBubbles2();
    }
